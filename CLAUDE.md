...
- **Controls:** keyboard (← → / A-D move, Space jump) + **gamepad** (`src/three/gamepad.ts`; 8BitDo SN30 Pro mapped — D-pad/stick move, A jumps). HUD has Reset + "Next world →" buttons; click the canvas to focus.
- **Debug:** `window.__weezy3d` — `getPlayer()`, `segments`, `jumpToSegment(i)`, `reset()`, `gamepad()`/`gamepadConnected()`, `scene`.
...
**Traversal powers are NOT yet ported to 3D** — power-gated segments are impassable, so only **Bedroom (gate-free) is fully playable** end-to-end; Hallway/Kitchen/Family Room/Backyard are playable up to their first gated seam. **Next big 3D milestone:** port the power system (playbook §5.4) and turn the companion cameos into real ability grants.
