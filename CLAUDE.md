...
**Painted-diorama art direction (2026-07-02, same branch, `?look=painted`):** after user verdict that the procedural look was "blah, not majestic," diagnosed as art direction (not an engine ceiling — Three.js can render this). Two playtest rounds shipped: (1) painted backdrop planes per segment + continuous edge-fading wallpaper (`src/three/paintedBackdrop.ts`), pit-aware dressing, bloom retune, dark under-floor for pit gaps (commit 79c3245); (2) after the user shared Mario Wonder references — diagnosis: Wonder's beauty is in the *terrain*, not the background — Wonder-style **quilted-blanket platforms with a plush carpet lip** replaced flat placeholder boxes (`WorldSurfaces.plush`, `buildPlushPlatform` in `level3d.ts`, commit 9df97fb).

**Painted APPROVED + now the DEFAULT (2026-07-08, session 9):** quilt terrain passed playtest ("looks and feels great"). Painted is the default look wherever backdrop art exists (`?look=classic` = procedural opt-out; non-bedroom worlds stay procedural until their paintings land — `hasPaintedBackdrop` is the rollout switch). Recipe deepened same day: **rolling floor-top silhouettes** (`rollingProfile.ts` pure + plush ExtrudeGeometry ridges behind the walk line, physics untouched), **fluttering wallpaper butterflies** (`butterflyPath.ts` pure + flock view in `paintedBackdrop.ts`), and — after a flat-cutout parallax prop layer was tried and REJECTED in-session ("the 3D blocks are more alive than these flat images") — **beautified 3D toys in painted mode**: lettered alphabet blocks, candy beach-ball stripes, candy foreground crumbs (`bedroomSet.ts`). 477 tests, tsc clean, browser-verified. Lessons + rejected-ingredient record in playbook §5.7. Next: roll the recipe to Hallway/Kitchen/FamilyRoom/Backyard; WebAudio blips still queued.
