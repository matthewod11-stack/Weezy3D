...
**Painted-diorama experiment (2026-07-02, same branch — `?look=painted`, bedroom only, awaiting user verdict).** After the user's "blah" verdict on the procedural look, the Eloise recipe (painted art in 3D space) was applied to the room itself: 5 NanoBanana-pro backdrop paintings style-locked to `docs/reference-art/Bedroom.jpg` (one per segment, `assets/backdrops3d/bedroom/`, ~180KB JPEGs), hung as fog-free MeshBasic planes at wallpaper depth with mirrored-repeat tiling (`paintedBackdrop.ts`); the procedural wall layer + mid-ground landmarks sit out (painted furniture in front of a painting reads as cardboard); floor/platform/clutter palettes shift candy; a gentle UnrealBloomPass (0.35/0.5/0.85) makes the fairy lights and Eloise's rim glow. Generation prompt pattern that worked first-shot: reference image attached + "flat side-on orthographic wall view, carpet floor line at exactly 15% from bottom, edges seamlessly continuable, NO characters, NO text, slightly soft/hazy background-art finish" + `use_image_history` for cross-segment consistency. **If approved:** per-world = 5 paintings + a tileable wallpaper swatch + a `BACKDROP_URLS`/`WALLPAPER_URLS` entry + the set's `paintedWall` opt — the old 2D-era `assets/backgrounds/` pixel-art paintings are style-mismatched, do NOT reuse them. Bloom is the one real frame cost — first lever to pull if the user reports chop.

**APPROVED + DEFAULT (2026-07-08).** The quilt terrain passed playtest ("looks and feels great") and painted became the DEFAULT look wherever a world has backdrop art — `?look=classic` is the procedural opt-out; worlds without paintings fall back automatically (`hasPaintedBackdrop` is the rollout switch). Two more recipe ingredients shipped the same day: **rolling floor-top silhouettes** (`rollingProfile.ts` pure + 7 tests → plush ExtrudeGeometry ridges per floor in `level3d.ts`, z-span [−1.6, −0.5] — BEHIND the shadow blob at −0.3, physics untouched) and **fluttering wallpaper butterflies** (`butterflyPath.ts` pure + 6 tests → ~5–9 two-wing flocks at −6.9 in `paintedBackdrop.ts`, ticked off the main loop's `elapsed`).

**Parallax cutout props: TRIED AND REJECTED (2026-07-08, same session).** The third named ingredient — painted furniture cutouts (NanoBanana→rembg RGBA planes) at −5.2/−2.8 — shipped, playtested, and was cut in one round: *"the 3D blocks etc are more alive than these flat images — if anything those should be beautified."* Two paid-for lessons: (a) **flat billboards near the play plane lose to real lit geometry** — the painted-art-in-3D-space recipe works for Eloise (animated character) and the far wallpaper plane, NOT for static mid-depth objects sitting next to actual meshes; (b) **scale fiction: Eloise is SHRUNK** — room furniture must tower (~8 of her) and toys are her peers, never trinkets; "correctly"-proportioned furniture reads as dollhouse miniatures. What replaced the props: **beautify the existing 3D toys in painted mode** — lettered alphabet-block faces (canvas texture, cached per letter+color, "E" for Eloise in rotation), candy beach-ball gore stripes, candy marbles/crayons/jacks in the foreground crumbs — all with the quilt's emissiveMap lift. The prop generation recipe (reference-image style lock + "single object, plain background" + `scripts/remove-bg.py` u2netp) is documented here because it WORKS as a pipeline — just point it at a use case where flat art belongs (far planes, paintings, UI).

**Playtest round 2 learnings (2026-07-02, commit 79c3245):** (a) mirror-tiling is for PATTERNS only — tiling a furniture composition clones the bed; the shipped shape is a continuous tinted wallpaper layer + vignette paintings on a ~34-unit cadence cycling in order, each alpha-edge-fading into the wallpaper (no seams, no clones). (b) Tint-multiply the wallpaper (0xf0cfda) or its whites catch bloom as kaleidoscope blotches. (c) Bloom 0.22/0.4/0.92 — at 0.85 threshold the lamp fireballs and Eloise's white dress blooms. (d) Pits need `buildUnderFloor` (dark gradient inside the floor z-range) or they show page background. (e) Dressing must be pit-aware — `buildBedroomSet` takes `floors` ranges from main (h≥55 tell); lamp+rug snap to a wide range. User verdict after round 2: improved but still "a bunch of images next to each other" — the identified next step was PARALLAX layering (painted cutout props at multiple depths via the remove-bg pipeline), not more single-plane art. *(Superseded 2026-07-08 — cutout props were tried and REJECTED, see the paragraph above; the parallax-depth idea survives only as beautified real 3D geometry.)*
